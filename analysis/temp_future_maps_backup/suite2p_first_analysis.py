...
# %% BEHAVIORAL DATA
path = "/home/georg/data/local_imaging/2023-02-02_13-46-16_twodistributionsv6_GR/"
...
s2p_folder = Path(tiffs_folder) / "suite2p" / "plane0"
...
frame_timestamps = bhv.correct_wraparound_np(np.array(frame_timestamps))
trial_timestamps = bhv.correct_wraparound_np(np.array(trial_timestamps))
...
frames_timestamps_corr = frames_timestamps_corr[:D.shape[0]]
...
for delay in delays:
    ...
    axes.matshow(S.T,vmin=vmin,vmax=vmax,extent=extent)
